...
How **fwxray** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `fwxray` lives in the **Malware / binary** cluster.
...
| from | relation | to |
|---|---|---|
| `fwxray` | JSON findings compose with | [`yaragen`](https://github.com/cognis-digital/yaragen), [`binhunt`](https://github.com/cognis-digital/binhunt), [`entropyscan`](https://github.com/cognis-digital/entropyscan) |
| `fwxray` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `fwxray` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `fwxray` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the malware / binary cluster.** `fwxray` output feeds the next tool:
```bash
fwxray ... --format json > out.json          # this tool's findings (see `fwxray --help`)
...
fwxray ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
fwxray ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
fwxray ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `fwxray`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `fwxray` slots
